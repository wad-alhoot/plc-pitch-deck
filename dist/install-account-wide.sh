#!/usr/bin/env bash
# ============================================================================
#  claude-code-power-stack — تنصيب على مستوى الحساب (كل المشاريع على هذا الجهاز)
#  الاستخدام:  bash install-account-wide.sh
#  يُنشئ: ~/.claude/skills/claude-code-power-stack/
#  ويُضيف طبقة الذاكرة إلى: ~/.claude/CLAUDE.md   (مع نسخة احتياطية)
# ============================================================================
set -euo pipefail

SKILL_DIR="$HOME/.claude/skills/claude-code-power-stack"
MEM_FILE="$HOME/.claude/CLAUDE.md"
BEGIN_MARK="<!-- BEGIN claude-code-power-stack -->"
END_MARK="<!-- END claude-code-power-stack -->"

mkdir -p "$SKILL_DIR/reference"

cat > "$SKILL_DIR/SKILL.md" <<'__PS_SKILL_EOF__'
---
...
cat > "$SKILL_DIR/reference/install.md" <<'__PS_REF_EOF__'
...
echo "✓ المهارة: $SKILL_DIR"

if [ -f "$MEM_FILE" ] && grep -qF "$BEGIN_MARK" "$MEM_FILE"; then
  echo "• طبقة الذاكرة موجودة مسبقاً في $MEM_FILE — يجري تحديثها"
  cp "$MEM_FILE" "$MEM_FILE.bak.$(date +%Y%m%d%H%M%S)"
  awk -v b="$BEGIN_MARK" -v e="$END_MARK" '
    index($0,b){skip=1} !skip{print} index($0,e){skip=0}
  ' "$MEM_FILE" > "$MEM_FILE.tmp" && mv "$MEM_FILE.tmp" "$MEM_FILE"
elif [ -f "$MEM_FILE" ]; then
  cp "$MEM_FILE" "$MEM_FILE.bak.$(date +%Y%m%d%H%M%S)"
  echo "• نسخة احتياطية من الذاكرة الحالية محفوظة"
fi
...
echo "✓ الذاكرة العامة: $MEM_FILE"
echo
echo "الخطوة التالية — نصّب الإضافات داخل جلسة Claude Code:"
...
echo "  /plugin install claude-mem      # ثم أعد تشغيل Claude Code"
echo
echo "و gstack من الـ terminal:"
