...
description: >-
  حزمة القوة السداسية لـ Claude Code — الطبقات الست التي تُحوّل Claude Code من
  «أوتوكمبليت متطوّر» إلى فريق تطوير كامل: تخطيط (Superpowers)، تصميم
  (Frontend Design)، مراجعة كود (Code Review)، أمان (Security Review)، ذاكرة بين
  الجلسات (Claude-Mem)، وستاك المنتج (gstack). استخدم هذه المهارة دائماً
  وبشكل تلقائي عند: بدء أي ميزة أو مشروع برمجي جديد، العودة إلى مشروع قديم بعد
  انقطاع، طلب «خطة» أو «plan» أو «ابدأ التنفيذ»، قبل فتح أي PR أو نشر أي كود،
  عند لمس المصادقة أو البيانات أو أي مدخلات خارجية، عند بناء أو تعديل واجهة
  أمامية، عند السؤال عن plugins أو skills أو marketplace أو workflow أو
  «كيف أرتّب شغلي مع Claude Code»، وعند الحاجة لتنسيق عدة مهارات مع بعضها في
  مهمة واحدة. المهارة تعمل كطبقة تنسيق (orchestration layer) فوق بقية المهارات:
  تُحدّد أي مهارة تعمل في أي مرحلة وبأي ترتيب، وتمنع تكرار العمل أو تضاربه.
  لا تتجاهلها حتى لو بدت المهمة صغيرة — أغلب فشل الكود المُولَّد بالذكاء
  الاصطناعي سببه غياب الترتيب لا ضعف الموديل.
---

# حزمة القوة السداسية لـ Claude Code

## المبدأ الحاكم

الإضافات وحدها ليست الفكرة. **الفكرة هي ترتيبها على دورة البناء: خطّط ← ابنِ ← اطلق.**
لا واحدة من هذه الطبقات تُعوّض عن حكمك — هي فقط تُثبّت الـ workflow الذي يمشي عليه
المهندس الشاطر أصلاً: خطّط قبل ما تبني، راجع قبل ما تنشر، وتذكّر شو قررت الأسبوع الماضي.

القاعدة الصفرية: **لا سطر كود قبل spec متّفق عليه، ولا PR قبل مراجعة، ولا نشر قبل فحص أمني.**
...
مفصّلة في `reference/install.md`. المختصر:
